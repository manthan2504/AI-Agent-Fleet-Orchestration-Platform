...
> **Note:** this checklist corresponds to Phase 6 of `docs/roadmap.md`'s implementation roadmap (Foundation → Memory → Security → Task Infrastructure → Observability → **Fleet Dashboard** → Scale). The dashboard's dependencies — it needs Task Infrastructure and Observability substantially built first — are explicit there, not assumed.