...
| `approval_config` | object: `title`, `description`, `severity`, `timeout`, **`designated_approver` (required, never agent-supplied — same treatment as `severity`/`risk_level` below)** | required if `decision = REQUIRE_APPROVAL`, and `designated_approver` itself is required within it, not optional | Feeds the `ApprovalRequest` created when this policy fires (§8.7.1). A policy that omits `designated_approver` is invalid configuration, rejected at write time — the natural implementer fallback for a missing value ("any user with the approver role") is exactly the approver-shopping this field exists to prevent, so there is no permitted default to fall back to. |
...
- **The six-class floor's classification isn't fully covered by matrix columns yet — flagging the gap rather than papering over it.** The matrix (§8.3.1) only has columns for 2 of the 6 floor classes (`Database (write)`, `Cloud/Infra deploy`); credential rotation, financial transaction, external communication, and infrastructure destruction (as distinct from deploy) have no matrix column to compile from. Every `tool_id` needs a `gated_action_class` tag (one of the six, or none) maintained server-side as part of the tool's own registration — never agent-writable, same treatment as `risk_level` and `designated_approver` above — so the floor has something to evaluate for tools outside the two currently-mapped categories. Until that tagging exists for all six classes, the floor is only mechanically enforced for the two categories the matrix already covers.
